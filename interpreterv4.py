        ...
        if kind == 'int' or kind == 'string' or kind == 'bool':
            ...
            if kind == '&&': # short circuit
                ...
                if tl == bool:
                    if not l: # l is false
                        return False
                    r = self.run_expr(expr.get('op2')) # l is true
                    tr = type(r)
                    if tr == bool:
                        return r
                super().error(ErrorType.TYPE_ERROR, '&& wrong type')
            ...
            if kind == '||': # short circuit
                ...
                if tl == bool:
                    if l: # l is true
                        return True
                    r = self.run_expr(expr.get('op2')) # l is false
                    tr = type(r)
                    if tr == bool:
                        return r
                super().error(ErrorType.TYPE_ERROR, '&& wrong type')
            ...
            l, r = self.run_expr(expr.get('op1')), self.run_expr(expr.get('op2')) # this runs both exprs, which we don't want for short circuiting
            tl, tr = type(l), type(r)
